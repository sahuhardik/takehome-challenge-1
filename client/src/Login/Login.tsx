...
export const Login = ({ onLoggedIn }: ILogin) => {
  ...
  const handleAuthenticate = ({ publicAddress, signature }: { publicAddress: any; signature: any }) =>
  ...
  const handleSignMessage = async ({ publicAddress, nonce }: { publicAddress: string; nonce: string }) => {
    try {
      const signature = await web3?.eth.personal.sign(`nonce: ${nonce}`, publicAddress, '');
  ...
  const handleClick = async () => {
    ...
    fetch(`${process.env.REACT_APP_BACKEND_URL}/users?publicAddress=${publicAddress}`)
      ...
      // Pass accessToken back to parent component (to persist it client side)
      .then(onLoggedIn)
      .catch((err) => {
        // We can implement any alert snackbar for good ux.
        window.alert(err);
        setLoading(false);
      });
  };
